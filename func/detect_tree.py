...
def tree_detection(seg_onehot, search_range=2, branch_penalty=16., pixdim_info=None, need_skeletonize_3d=True):
    center_map, center_dict, nearby_dict = get_the_skeleton_and_center_nearby_dict(
        seg_onehot, search_range=search_range, need_skeletonize_3d=need_skeletonize_3d)
    connection_dict = get_connection_dict(
        center_dict, nearby_dict, pixdim_info, branch_penalty)
...
def get_the_skeleton_and_center_nearby_dict(seg_input, search_range=10, need_skeletonize_3d=True):
    ...
    for i in range(len(center_locs[0])):
        center_dict[i+base_count] = [center_locs[0]
                                     [i], center_locs[1][i], center_locs[2][i]]
        center_map[center_locs[0][i], center_locs[1]
                   [i], center_locs[2][i]] = i+base_count
...
def get_connection_dict(center_dict, nearby_dict, pixdim_info=None, branch_penalty=16.):
    ...
    if pixdim_info is None:
        ...
        pixdim = np.array(
            [pixdim_info['pixdim_z'], pixdim_info['pixdim_x'], pixdim_info['pixdim_y']])
    ...
    def find_connection(current_label, before_label):
        ...
        for nearby_label in nearby_labels:
            if connection_dict[nearby_label]["is_processed"] == False:
                ...
                dist_to_valid_labels.append(np.sum(((np.array(
                    connection_dict[nearby_label]["loc"])-np.array(connection_dict[current_label]["loc"]))**2)))
            else:
                processed_count += 1
        ...
        if len(valid_next_labels) == 0 or len(nearby_labels) == processed_count:
            ...
            for valid_next_label in valid_next_labels:
                if connection_dict[valid_next_label]["is_processed"] == False:
                    connection_dict[current_label]["next"].append(
                        valid_next_label)
        ...
        while len(pq) > 0:
            ...
            if before_label != null_label and len(connection_dict[before_label]["next"]) <= 2:
                ...
                for nearby_label in nearby_labels:
                    if connection_dict[nearby_label]["is_processed"] == False:
                        valid_next_labels.append(nearby_label)
                        dir_vec = (np.array(connection_dict[nearby_label]["loc"]) - np.array(
                            connection_dict[before_label]["loc"])) * pixdim
                        ...
                        heapq.heappush(
                            pq, (cost, true_outdegree + 1, before_label, nearby_label))
    ...
    find_connection_using_regularized_prim(
        start_label=slice_idxs[0], null_label=0)

    for item in connection_dict.keys():
        assert len(connection_dict[item]["before"]
                   ) <= 1, (item, connection_dict[item])
        connection_dict[item]["number_of_next"] = len(
            connection_dict[item]["next"])
        connection_dict[item]["is_bifurcation"] = (
            connection_dict[item]["number_of_next"] > 1)
...
def get_tree_length(connection_dict, is_3d_len=True):
    ...
    def get_tree_length_func(connection_dict, current_label):
        ...
        if connection_dict[current_label]["number_of_next"] == 0:
            ...
            for next_label in connection_dict[current_label]["next"]:
                if is_3d_len:
                    current_branch_length += np.sqrt(np.sum((np.array(
                        connection_dict[current_label]["loc"])-np.array(connection_dict[next_label]["loc"]))**2))
                else:
                    current_branch_length += 1
            print("len of "+str(current_label)+" branch is " +
                  str(current_branch_length), end="\r")
    ...
    def dfs(current_label: int) -> None:
        ...
        if connection_dict[current_label]["number_of_next"] > 0:
            for next_label in connection_dict[current_label]["next"]:
                ...
                dir_vec = (np.array(connection_dict[current_label]["loc"]) - np.array(
                    connection_dict[next_label]["loc"])) * pixdim
                segment_dict[next_segment_no]["length"] += np.sqrt(
                    np.sum((dir_vec)**2))
                if connection_dict[current_label]["is_bifurcation"]:
                    segment_dict[current_segment_no]["next"].append(
                        next_segment_no)
...
def get_subtree_length(segment_dict: dict) -> dict:
    for key in segment_dict.keys():
        ...
        segment_dict[key]["subtree_depth"] = 0.

    def tree_dp(current_segment: int) -> float:
        nonlocal segment_dict
        for next_segment in segment_dict[current_segment]["next"]:
            next_segment_length, next_segment_depth = tree_dp(next_segment)
            segment_dict[current_segment]["subtree_length"] += next_segment_length
            segment_dict[current_segment]["subtree_depth"] = max(
                segment_dict[current_segment]["subtree_depth"], next_segment_depth)

        segment_dict[current_segment]["subtree_length"] += segment_dict[current_segment]["length"]
        segment_dict[current_segment]["subtree_depth"] += segment_dict[current_segment]["length"]

        return segment_dict[current_segment]["subtree_length"], segment_dict[current_segment]["subtree_depth"]
...
def prune_segment(segment_dict: dict, th_ratio: float = 0.05, max_gen: Optional[int] = None) -> dict:
    ...
    for segment, val in segment_dict.items():
        ...
        if val["before"] == 0:
            continue
        parent = val["before"]
        siblings_subtree_length_sum = segment_dict[parent]["subtree_length"] - \
            segment_dict[parent]["length"]
...
def prune_segment_palagy(segment_dict: dict, segment_array: np.ndarray, t_l: float = 3., t_d: float = 1.5, pixdim_info=None, max_gen: Optional[int] = None) -> dict:
    def prune_subtree(current_segment):
        nonlocal segment_dict
        if segment_dict[current_segment]["pruned"]:
            return
    ...
    for segment, val in segment_dict.items():
        ...
        parent = val["before"]
        croped = get_crop(
            segment_array, segment_dict[parent]["endpoint_loc"], search_range=6)

        if val["length"] < t_l and len(val["next"]) == 0:
            too_far_from_surface = True
            # center of croped is (croped.shape[0]//2, croped.shape[1]//2, croped.shape[2]//2)
            for i in range(croped.shape[0]):
                if not too_far_from_surface:
                    break
                for j in range(croped.shape[1]):
                    if not too_far_from_surface:
                        break
                    for k in range(croped.shape[2]):
                        dist = np.sqrt(
                            (i - croped.shape[0] // 2) ** 2 +
                            (j - croped.shape[1] // 2) ** 2 +
                            (k - croped.shape[2] // 2) ** 2
                        )
                        # if dist is close to the surface and the voxel is empty
                        if not bool(croped[i, j, k]) and dist < t_d:
                            too_far_from_surface = False
                            break

            if too_far_from_surface:
                prune_subtree(segment)
...
def prune_conneciton_dict(connection_dict: dict, th_ratio: float = 0.05, pixdim_info=None, max_gen: Optional[int] = None):
    segment_dict = get_segment_dict(connection_dict, pixdim_info)
    segment_dict = get_subtree_length(segment_dict)
    segment_dict = prune_segment(
        segment_dict, th_ratio=th_ratio, max_gen=max_gen)
    ...
    for current_label, val in connection_dict.items():
        ...
        connection_dict[current_label]["number_of_next"] = len(
            connection_dict[current_label]["next"])
    ...
    find_generation_and_node_no(current_label=list(
        connection_dict.keys())[0], generation=1)
    gc.collect()
    return connection_dict


def prune_conneciton_dict_palagy(connection_dict: dict, segment_array: np.ndarray, t_l: float = 3., t_d: float = 1.5, pixdim_info=None, max_gen: Optional[int] = None):
    segment_dict = get_segment_dict(connection_dict, pixdim_info)
    segment_dict = get_subtree_length(segment_dict)
    segment_dict = prune_segment_palagy(
        segment_dict, segment_array, t_l=t_l, t_d=t_d, pixdim_info=pixdim_info, max_gen=max_gen)
