...
from .detect_tree import tree_detection, prune_conneciton_dict, prune_conneciton_dict_palagy, get_trace_voxel_count_by_gen_from_root, get_segment_dict
...
def break_and_save(seg_path: str, save_path: str, generation_info: pd.DataFrame, trace_volume_by_gen_info: pd.DataFrame, trace_slice_area_info: pd.DataFrame, args, pixdim_info=None):
    ...
    csv_path = save_path.rstrip('/').rstrip('\\') + \
    ...
    seg_processed_II = sitk.GetArrayFromImage(
        sitk.ReadImage(seg_path)).astype(int)
    ...
    last_nonzero = np.argwhere(
        np.sum(seg_processed_II, axis=(1, 2)) > 0)[-1][0]
    seg_processed_II_clean[:last_nonzero -
                           CUTOFF_SLICE_COUNT] = seg_processed_II[:last_nonzero - CUTOFF_SLICE_COUNT]
    # The last index indicates top of slice actually
    for i in range(CUTOFF_SLICE_COUNT - 10):
        seg_processed_II_clean[last_nonzero - CUTOFF_SLICE_COUNT + i] = \
            ((get_only_largest_component(seg_processed_II_clean[last_nonzero - CUTOFF_SLICE_COUNT - 1 + i]) +
             seg_processed_II[last_nonzero - CUTOFF_SLICE_COUNT + i]) > 0).astype(seg_processed_II_clean.dtype)

    for i in range(CUTOFF_SLICE_COUNT - 10, 10000):
        if last_nonzero - CUTOFF_SLICE_COUNT + i >= seg_processed_II_clean.shape[0]:
            break
        seg_processed_II_clean[last_nonzero - CUTOFF_SLICE_COUNT + i] = \
            get_only_largest_component(
                seg_processed_II_clean[last_nonzero - CUTOFF_SLICE_COUNT - 1 + i])

    # os.makedirs(save_path.rstrip('/').rstrip('\\') + '/fuck/', exist_ok=True)
    # sitk.WriteImage(sitk.GetImageFromArray(seg_processed_II_clean.astype(np.uint8)),
    ...
    seg_slice_label_II, connection_dict_of_seg_II, number_of_branch_II, tree_length_II = tree_detection(
        seg_processed_II_clean, search_range=32, branch_penalty=args.branch_penalty, pixdim_info=pixdim_info)
    if args.use_palagy:
        for _ in range(2):
            connection_dict_of_seg_II = prune_conneciton_dict_palagy(
                connection_dict_of_seg_II, segment_array=seg_processed_II_clean, pixdim_info=pixdim_info)
    else:
        connection_dict_of_seg_II = prune_conneciton_dict(
            connection_dict_of_seg_II, th_ratio=args.prune_threshold, pixdim_info=pixdim_info)

    # assign voxel label and split left and right lung airway
    if args.use_bfs:
        voxel_by_generation = get_voxel_by_generation(
            seg_processed_II, connection_dict_of_seg_II)
        voxel_by_segment_no = get_voxel_by_segment_no(
            seg_processed_II, connection_dict_of_seg_II)
    else:
        voxel_by_generation = get_voxel_by_generation_without_bfs(
            seg_processed_II, connection_dict_of_seg_II)
        voxel_by_segment_no = get_voxel_by_segment_no_without_bfs(
            seg_processed_II, connection_dict_of_seg_II)

    segment_dict = get_segment_dict(connection_dict_of_seg_II, pixdim_info)

    voxel_by_generation_left, voxel_by_generation_right = get_left_and_right_lung_airway(
        voxel_by_generation, voxel_by_segment_no, connection_dict_of_seg_II, segment_dict)

    # get segment dict and find voxel count by generation on each trace starting from root
    voxel_count_by_segment_no = get_voxel_count_by_segment_no(
        voxel_by_segment_no, segment_dict)
    segment_dict = get_trace_voxel_count_by_gen_from_root(
        segment_dict, voxel_count_by_segment_no)
    ...
    if upside_down:
        ...
        for key, val in segment_dict.items():
            segment_dict[key]['endpoint_loc'][0] = seg_processed_II.shape[0] - \
                segment_dict[key]['endpoint_loc'][0] - 1
        for key, val in connection_dict_of_seg_II.items():
            connection_dict_of_seg_II[key]['loc'][0] = seg_processed_II.shape[0] - \
                connection_dict_of_seg_II[key]['loc'][0] - 1
    ...
    if pixdim_info is None:
        ...
        voxel_size = pixdim_info['pixdim_x'] * \
            pixdim_info['pixdim_y'] * pixdim_info['pixdim_z']
    ...
    for suffix, voxel in zip(('total', 'l', 'r'), (voxel_by_generation, voxel_by_generation_left, voxel_by_generation_right)):
        voxel_count_by_generation = get_voxel_count_by_generation(
            voxel, connection_dict_of_seg_II).astype(int)
        for j, voxel_count in enumerate(voxel_count_by_generation):
            if j == 0:
                continue
            if j == 10:
                # dict_row[str(j) + suffix] = voxel_count_by_generation[j:].sum() * voxel_size
                dict_row['_'.join(('volume', suffix, str(j)))
                         ] = voxel_count_by_generation[j] * voxel_size
                break
            else:
                dict_row['_'.join(('volume', suffix, str(j)))
                         ] = voxel_count * voxel_size

        dict_row['volume_sum_' +
                 suffix] = voxel_count_by_generation[1:11].sum() * voxel_size
        ...
        for j, voxel_count in enumerate(voxel_count_by_generation):
            ...
            if j == 10:
                # dict_row[str(j) + suffix] = voxel_count_by_generation[j:].sum() * voxel_size
                dict_row['_'.join(('volume_ratio', suffix, str(
                    j)))] = voxel_count_by_generation[j] / voxel_count_by_generation[1:11].sum()
                break
            else:
                dict_row['_'.join(('volume_ratio', suffix, str(
                    j)))] = voxel_count / voxel_count_by_generation[1:11].sum()
        ...
        for j in range(1, 11):
            dict_row['_'.join(('branch_count', suffix, str(j)))
                     ] = branch_count[j]

        for j in range(1, 11):
            if branch_length[j] < 0.0001:
                dict_row['_'.join(('branch_area', suffix, str(j)))] = float(
                    'nan')
            else:
                dict_row['_'.join(('branch_area', suffix, str(
                    j)))] = voxel_count_by_generation[j] * voxel_size / branch_length[j]
    ...
    for suffix, voxel in zip(('_total', '_l', '_r'), (voxel_by_generation, voxel_by_generation_left, voxel_by_generation_right)):
        ...
        dict_row["vol_of_bronchlole_at_the_last_branch" +
                 suffix] = voxel_count_by_generation[last_branch_observed] * voxel_size

    # --------------------------------

    # 24-01-03 요청사항 (1)
    # --------------------------------
    voxel_count_by_generation_left = get_voxel_count_by_generation(
        voxel_by_generation_left, connection_dict_of_seg_II).astype(int)
    dict_row["last_branch_observed_l"] = (
        voxel_count_by_generation_left > 0).astype(int).sum() + 1
    voxel_count_by_generation_right = get_voxel_count_by_generation(
        voxel_by_generation_right, connection_dict_of_seg_II).astype(int)
    dict_row["last_branch_observed_r"] = (
        voxel_count_by_generation_right > 0).astype(int).sum() + 1
    ...
    for key in segment_dict.keys():
        segment_dict[key]["volume"] = voxel_size * \
            voxel_count_by_segment_no[key]
        segment_dict[key]["area"] = segment_dict[key]["volume"] / \
            segment_dict[key]["length"]
        segment_dict[key]["diameter"] = 2 * \
            np.sqrt(segment_dict[key]["area"] / np.pi)
    ...
    seg_processed_II_extended_labeled = np.zeros_like(
        seg_processed_II_extended)
    for i in range(len(seg_processed_II_extended)):
        seg_processed_II_extended_labeled[i] = measure.label(
            seg_processed_II_extended[i], connectivity=1)
    ...
    for key, val in segment_dict.items():
        if val['generation'] == 10 or (val['generation'] < 10 and len(val['next']) == 0):
            ...
            if pixdim_info is not None:
                one_pixel_area = pixdim_info['pixdim_x'] * \
                    pixdim_info['pixdim_y']
            else:
                one_pixel_area = 1.
            dict_row['endpoint_area'] = (seg_processed_II_extended_labeled[val['endpoint_loc'][0]] ==
                                         seg_processed_II_extended_labeled[val['endpoint_loc'][0], val['endpoint_loc'][1], val['endpoint_loc'][2]]).sum() * one_pixel_area
            dict_row['endpoint_diameter'] = 2 * \
                np.sqrt(dict_row['endpoint_area'] / np.pi)
            ...
            for i, cnt in enumerate(val['trace_voxel_count_by_gen']):
                if i != 0:
                    ...
                    if pixdim_info is not None:
                        dict_row[f'{str(i)}'] *= pixdim_info['slice_count'] / \
                            extended_image_height
                    volume_sum += dict_row[f'{str(i)}']

            for i, cnt in enumerate(val['trace_voxel_count_by_gen']):
                if i != 0:
                    dict_row[f'{str(i)}_ratio'] = dict_row[f'{str(i)}'] / \
                        volume_sum

            # trace_volume_by_gen_info = pd.DataFrame(trace_volume_by_gen_info.append(dict_row, ignore_index=True))
            trace_volume_by_gen_info = trace_volume_by_gen_info.append(
                dict_row, ignore_index=True)
    ...
    for key, val in segment_dict.items():
        if val['generation'] == 10 or (val['generation'] < 10 and len(val['next']) == 0):
            ...
            while True:
                ...
                dict_row['slice_diameter'] = 2 * \
                    np.sqrt(dict_row['slice_area'] / np.pi)

                trace_slice_area_info = trace_slice_area_info.append(
                    dict_row, ignore_index=True)
    ...
    generation_info.to_csv(save_path.rstrip(
        '/').rstrip('\\') + '/' + "generation_info.csv", index=False)
    print(generation_info)

    # save trace volume csv
    trace_volume_by_gen_info.to_csv(save_path.rstrip(
        '/').rstrip('\\') + '/' + "trace_volume_by_gen_info.csv", index=False)
    ...
    os.makedirs(save_path.rstrip('/').rstrip('\\') +
                f"/trace_slice_area_info/", exist_ok=True)
    trace_slice_area_info.to_csv(save_path.rstrip(
        '/').rstrip('\\') + f"/trace_slice_area_info/" + f"trace_slice_area_info_{seg_file_name}.csv", index=False)
    print(trace_slice_area_info)

    os.makedirs(save_path.rstrip('/').rstrip('\\') +
                '/centerline_dicts/', exist_ok=True)
    centerline_dict_path = \
        save_path.rstrip('/').rstrip('\\') + '/centerline_dicts/' + seg_path[seg_path.rfind(
            '/') + 1:seg_path.find('.')][seg_path.rfind('\\') + 1:] + '.pkl'
    with open(centerline_dict_path, 'wb') as f:
        pickle.dump(connection_dict_of_seg_II, f)

    for i in range(0, 10):
        os.makedirs(save_path.rstrip('/').rstrip('\\') +
                    '/high_gens/', exist_ok=True)
    ...
    df_of_line_of_centerline = get_df_of_line_of_centerline(
