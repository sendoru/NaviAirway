...
def get_connection_dict(center_dict, nearby_dict):
    ...
    def find_connection_using_regularized_prim(start_label, null_label=0):
        global connection_dict
        pq = []
        pq.append((-10000, null_label, start_label))
        while len(pq) > 0:
            _, before_label, current_label= pq[0]
            heapq.heappop(pq)
            ...
            for nearby_label in nearby_labels:
                if connection_dict[nearby_label]["is_processed"] == False:
                    ...
                    if len(connection_dict[current_label]["next"]) >= 2:
                        cost -= 16
                    elif len(connection_dict[current_label]["next"]) == 1:
                        cost += 16
                    heapq.heappush(pq, (cost, current_label, nearby_label, ))
                else:
                    processed_count+=1
